...
import { getAllAddStoryChapters, addStoryChapters } from '@/app/data/addStoryChapters';
...
export default function AddStoryPage() {
  const allChapters = getAllAddStoryChapters();
        ...
        <div className="max-w-4xl mx-auto px-4">
          <div className="space-y-3">
            {allChapters.map((chapter, index) => (
              <AddStoryChapterItem key={chapter.id} chapter={chapter} index={index} />
            ))}
          </div>
        </div>
...
function AddStoryChapterItem({ chapter, index }: { chapter: typeof addStoryChapters[0]; index: number }) {
  const { ref, isVisible } = useScrollAnimation();

  return (
    <Link
      ref={ref as any}
      href={`/AddStory/${chapter.id}`}
      className={`
